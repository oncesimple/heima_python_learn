    ...
    for i in video_list:
        ...
        if len(video_list) > 1:
            urllib.request.urlretrieve(url=i, filename=os.path.join(currentVideoPath, r'{}-{}.mp4'.format(title, num)),
                                       reporthook=Schedule_cmd)  # 写成mp4也行  title + '-' + n + '.flv'
        else:
...
def do_prepare(inputStart, inputQuality):
    ...
    if start.isdigit() == True:  # 如果输入的是BV号
        ...
        start_url = 'https://api.bilibili.com/x/web-interface/view?bvid=' + re.search(r'BV([0-9a-zA-Z]*)', start).group(1)
    print(start_url)
    # 视频质量
...
if __name__ == "__main__":
    ...
    labelStart = Label(root, text="请输入您要下载的B站BV号或者视频链接地址:")  # 地址输入
    ...
    # 可供选择的表
    inputQual['value'] = ('1080P+','1080P', '720p', '480p', '360p')
    # 对应的转换字典
    keyTrans = dict()
    keyTrans['1080P+'] = '112'
    keyTrans['1080P'] = '80'
    ...
    msgbox = Text(root)
    msgbox.insert('1.0',
                  "对于单P视频:直接传入B站BV号或者视频链接地址\n(eg: 49842011或者https://www.bilibili.com/video/BV18E411p7j8)\n对于多P视频:\n1"
                  ".下载全集:直接传入B站BV号或者视频链接地址\n(eg: "
                  "49842011或者https://www.bilibili.com/video/BV18E411p7j8)\n2.下载其中一集:传入那一集的视频链接地址\n(eg: "
                  "https://www.bilibili.com/video/BV18E411p7j8?p=2)")
    msgbox.pack()
